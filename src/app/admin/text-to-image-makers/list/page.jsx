...
function Page() {
    ...
    const fetchData = useCallback(async (query = "", page = 1) => {
        try {
            ...
            const res = await axios.get(
                `/api/text-to-image?search=${query}&pageNumber=${page}&pageSize=${perPage}`,
                {
    ...
    const confirmDelete = async () => {
        if (!deleteId) return;
        try {
            const res = await axios.delete(`/api/text-to-image/${deleteId}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
                ...
                <div className="flex-1 mt-[68px] lg:ml-64 ml-20 p-6">
                    <div className="bg-white/80 backdrop-blur-lg rounded-xl shadow-2xl p-6">
                        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                            <h2 className="text-3xl font-semibold text-gray-800">Text to Image Makers</h2>
