  ...
  return (
    <>
      <nav className="fixed top-0 left-0 w-full bg-foreground z-50 shadow-md">
        <div
          className={`max-w-screen-xl mx-auto flex flex-wrap items-center ${
            props.admin ? "py-4 px-2" : "p-4"
          }`}
        >
          ...
          <button
            onClick={toggleMenu}
            type="button"
            className={`inline-flex items-center p-2 w-10 h-10 justify-center text-white rounded-lg md:hidden hover:bg-foreground focus:outline-none focus:ring-2 focus:ring-gray-500 ${
              props.admin ? "hidden" : ""
            }`}
          ...
          >
            <ul
              className={`flex flex-col mt-4 md:flex-row md:mt-0 md:space-x-5 ${
                props.admin ? "hidden" : ""
              }`}
            >
              {[
                ...
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className={`block py-2 custom-navLink-size ${
                      pathname === item.href
              ...
              {token && role === "user" ? (
                <li key="welcome" className="flex items-center space-x-2">
                  <div className="relative">
                    <button
                      ...
                      className="text-white font-semibold px-4 py-2 rounded hover:bg-gray-700 focus:outline-none"
                    >
                      Welcome, {fullName}
                    </button>
                ...
                <li key="/auth/login">
                  <Link href="/auth/login" className="inline-block">
                    <button className="bg-white text-foreground font-semibold px-4 py-1 rounded hover:bg-gray-200 focus:outline-none">
                      Login
                    </button>
                  </Link>
                </li>
